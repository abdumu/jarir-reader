...
const { nl2br, uuid, escapeOurXml } = require("./helpers");
...
const bookEpubGenerator = (book, info) => {
    return new Promise((resolve, reject) => {
        ...
        const option = {
            ...
            css: readFileSync(pathResolve(__dirname, "../", "public", "template.css"))
                .toString()
                .replace(/{{direction}}/g, (info.language || "ar") === "en" ? "left" : "right"),
    ...
    return new Promise((resolve, reject) => {
        ...
        for (let index = 1; index <= info.chapters; index++) {
            ...
            try {
                var text = readFileSync(
                    pathResolve(getAppDataPath("jarir-cli"), "books", "" + book.id, "Text", "chapter-" + String(index).padStart(3, 0) + ".htmlx")
                ).toString();
                var spans = readFileSync(
                    pathResolve(getAppDataPath("jarir-cli"), "books", "" + book.id, "Text", "chapter-" + String(index).padStart(3, 0) + ".html.spans")
                ).toString();
        ...
        try {
            content.push({
                ...
                filename: "copyrights",
                data: nl2br(parseSpan(book, readFileSync(pathResolve(__dirname, "../", "public", "last-chapter.txt")).toString(), [[0, 5, 0]]), true),
            });
            // console.log(content)
        } catch (err) {
            reject(err);
