...
const bookAudioGenerator = (book, info) => {
    return new Promise((resolve, reject) => {
        ...
        for (let index = 1; index <= info.chapters; index++) {
            var currentTocItem = toc.length >= index ? toc[index - 1] : {
                title: `chapter-${String(index).padStart(2, 0)}`,
                length: 0
            };
            var filename = `${book.authors.join('+').replace('_', '-')}_${cleanFilename(book.title, ' ')}_${String(index).padStart(2, 0)}_${currentTocItem['title']}.mp3`;
            ...
            try {
                renameSync(
                    pathResolve(getAppDataPath('jarir-cli'), 'books', book.id, 'Audio', `chapter-${String(index).padStart(3, 0)}.datx`),
                    newPath
                );
            } catch (e) {
